...
class BioSyn():
    ...
    def load_dense_encoder(self, bert_dir):
        ...
        self.dense_encoder = AutoModel.from_pretrained(bert_dir)
    ...
    def get_dense_representation(self,mentions,verbose=False):
        ...
        with torch.no_grad():
            for batch in tqdm(name_dataloader, disable=not verbose, desc='get dense representation'):
                ...
                outputs = self.dense_encoder(**batch)
                # 但是不知道为啥不选择outputs[1]作为[CLS]的output...
                batch_dense_embeds = outputs[0][:,0].cpu().detach().numpy()  # [CLS] representations,shape=(batch_size,hidden_size)=(1024,768)
