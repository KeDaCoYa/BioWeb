...
logger = logging.getLogger('main.re.entity_marker')
...
class MTBDataset(Dataset):
    def __init__(self, examples, config, tokenizer: BertTokenizer, label2id, device):
        '''
    ...
    def _process_seq_len(self, text_a, text_b, total_special_toks=3):
        ...
        while len(self.tokenizer.tokenize(text_a) + self.tokenizer.tokenize(text_b)) > (
                self.config.max_len - total_special_toks):
    ...
    def _truncate_helper(self, text):
        ...
        tags_li = [self.config.ent1_start_tag, self.config.ent1_end_tag, self.config.ent2_start_tag,
                   self.config.ent2_end_tag]
