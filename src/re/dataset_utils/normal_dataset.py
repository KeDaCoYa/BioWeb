...
class NormalDataset(Dataset):
    ...
    def collate_fn_predicate(self, features):
        ...
        batch_token_type_ids = torch.tensor(sequence_padding(batch_token_type_ids,length=pad_length),device=self.device).long()
        batch_attention_masks = torch.tensor(sequence_padding(batch_attention_masks,length=pad_length),device=self.device).long()
